...
type InspectorValueKind = 'hardcoded' | 'variable' | 'style';

interface InspectorTokenRef {
  id: string;
  name: string;
  collectionId?: string;
...
function formatInspectorColor(c: { r: number; g: number; b: number; a?: number }, opacity?: number): string {
  const toHex = (n: number) => ('0' + Math.round(Math.max(0, Math.min(1, n)) * 255).toString(16).toUpperCase()).slice(-2);
  const hex = '#' + toHex(c.r) + toHex(c.g) + toHex(c.b);
  const a = opacity !== undefined ? opacity : (c.a !== undefined ? c.a : 1);
  return a < 1 ? hex + toHex(a) : hex;
}

async function resolveVariableAlias(alias: any): Promise<InspectorTokenRef | null> {
...
async function resolveStyleName(styleId: any): Promise<string | null> {
...
async function buildPaintEntries(node: any, propName: 'fills' | 'strokes', category: string): Promise<InspectorEntry[]> {
...
const INSPECTOR_SCALAR_PROPS: Array<{ key: string; category: string; label: string }> = [
...
const INSPECTOR_TEXT_SCALAR_PROPS: Array<{ key: string; category: string; label: string }> = [
...
function formatInspectorScalarValue(value: any): string {
  if (typeof value === 'number') {
...
async function buildEffectEntries(node: any): Promise<InspectorEntry[]> {
  const entries: InspectorEntry[] = [];
...
async function addStyleEntry(node: any, key: string, category: string, label: string, entries: InspectorEntry[]) {
...
async function buildInspectorEntries(node: SceneNode): Promise<InspectorEntry[]> {
  ...
  if (node.type === 'TEXT') {
    ...
    if (anyNode.fontName !== figma.mixed) {
      const fontName = anyNode.fontName as FontName;
  ...
  if ('fillStyleId' in anyNode) await addStyleEntry(anyNode, 'fillStyleId', 'Fill', 'Fill Style', entries);
  if ('strokeStyleId' in anyNode) await addStyleEntry(anyNode, 'strokeStyleId', 'Stroke', 'Stroke Style', entries);
  if ('effectStyleId' in anyNode) await addStyleEntry(anyNode, 'effectStyleId', 'Effects', 'Effect Style', entries);
...
function buildGroupVariableName(currentName: string, currentGroupName: string, nextGroupName: string): string {
...
function buildTemporaryGroupVariableName(variable: Variable, index: number): string {
...
function doesGroupNameExist(
  ...
  return variables.some(variable => {
    ...
    if (ignoredIds && ignoredIds.has(variable.id)) return false;
    return variable.name === groupName || variable.name.indexOf(prefix) === 0;
  });
}

function buildDuplicateGroupName(
  variables: Variable[],
  collectionId: string,
  groupName: string
): string {
...
async function renameGroup(variableIds: string[], currentGroupName: string, nextGroupName: string) {
  try {
    ...
    const groupVariables: Array<{ variable: Variable; originalName: string }> = [];
...
async function duplicateGroup(variableIds: string[], groupName: string) {
  try {
    ...
    const duplicatedVariables = new Map<string, Variable>();

    sourceVariables
      .slice()
      .sort((a, b) => a.name.localeCompare(b.name))
      .forEach(variable => {
        const duplicatedVariable = figma.variables.createVariable(
          buildGroupVariableName(variable.name, groupName, nextGroupName),
          collection,
    ...
    sourceVariables.forEach(variable => {
      ...
      Object.keys(variable.valuesByMode).forEach(modeId => {
        ...
        if (
          ...
          'type' in sourceValue &&
          (sourceValue as any).type === 'VARIABLE_ALIAS'
        ) {
          const aliasTarget = duplicatedVariables.get((sourceValue as any).id);
          if (aliasTarget) {
            nextValue = {
              type: 'VARIABLE_ALIAS',
              id: aliasTarget.id,
...
function isVariableAliasValue(value: VariableValue | undefined): value is VariableAlias {
  return typeof value === 'object' && value !== null && 'type' in value
    && (value as VariableAlias).type === 'VARIABLE_ALIAS';
}

async function rebindMovedVariablesOnCanvas(oldToNew: Map<string, Variable>) {
  // documentAccess is dynamic-page: pages must be loaded before walking.
  await figma.loadAllPagesAsync();

  const rebindNode = (node: SceneNode) => {
    const anyNode = node as any;
    const bound = anyNode.boundVariables as Record<string, VariableAlias | VariableAlias[]> | undefined;
    if (bound) {
    ...
    if ('children' in node) {
      (node as ChildrenMixin).children.forEach(child => rebindNode(child as SceneNode));
    }
  };
...
async function safeMoveVariablesToCollection(variableIds: string[], targetCollectionId: string) {
  ...
  const toMove = variableIds
    ...
    .filter((v): v is Variable => !!v && v.variableCollectionId !== targetCollectionId);
  ...
  const sourceCollections = new Map<string, VariableCollection>();
  for (const variable of toMove) {
    if (!sourceCollections.has(variable.variableCollectionId)) {
      const collection = await figma.variables.getVariableCollectionByIdAsync(variable.variableCollectionId);
      if (collection) sourceCollections.set(variable.variableCollectionId, collection);
    }
  }

  // Target mode → source mode: same name first (Desktop→Desktop), then same
  // position, then the source's first mode.
  const modeMapFor = (source: VariableCollection): Map<string, string> => {
  ...
  const oldToNew = new Map<string, Variable>();
  for (const oldVar of toMove) {
    ...
    const modeMap = source ? modeMapFor(source) : new Map<string, string>();
  ...
  for (const variable of allVariables) {
    if (oldToNew.has(variable.id)) continue;
